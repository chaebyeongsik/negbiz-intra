...
var opUpdateUseYn = function(el, bbsSn, seCdId) {
    ...
    if(elementz.attr("id") == "useYn" && useYnVal == "N") {
    ...
    if(isProcessing) {
        $.ajax({
            url : "/intra/bbsMngr/ND_updateUseYn.do",
            type : "POST",
            data : {
                bbsSn : bbsSn,
                seCdId : seCdId,
 ...
 * 분류코드 그룹 폼 삭제
 */
var delClsfNo = function(obj) {
    $(obj).parent().parent().parent().remove();
};


/* 썸네일 생성 팝업 */
var opBbsCreateThumb = function(bbsSn) {
    ...
    var href = "PD_createThumbNail.do?q_bbsSn=" + bbsSn;
    var opBbsCreateThumbWin = window.open(href, "opBbsCreateThumb", option);
    opBbsCreateThumbWin.focus();
